...
enrich_tier2_llm.py
---------------------
Generalized version of the Tier-1 enrichment script — works across all
14 Major Appliance categories found in Tier 2 (dryer, washer, fridge,
range, oven, cooktop, microwave, beverage center, coffee maker,
espresso machine, toaster, freezer, laundry center) instead of being
hardcoded to one product type.

This is the actual generalization test: the same architecture (system
prompt structure, manufacturer/distributor disambiguation, format
rules, JSON schema) is reused — only the specific category name is
injected per row.

v2 fix (15 Aug 2026): switched MODEL from "llama-3.3-70b-versatile" to
...
openai/gpt-oss-120b is Groq's recommended replacement. Tier 2 output
...
Run:
  python scripts/enrich_tier2_llm.py
"""
...
SKELETON_PATH = "raw_data/html/tier2_skeleton.json"
TIER2_RAW_PATH = "raw_data/html/tier2_major_appliances.json"
CATEGORY_MAP_PATH = "raw_data/html/tier2_category_map.json"
OUTPUT_PATH = "raw_data/html/tier2_llm_enriched.json"

API_KEY = os.environ.get("GROQ_API_KEY", "PASTE_YOUR_KEY_HERE")
# Using the heavier model for better quality since a fresh API key
# resolves the quota concern. Checkpointing below still protects
# against any rate-limit or crash regardless of which model is used.
MODEL = "openai/gpt-oss-120b"

client = Groq(api_key=API_KEY)

CATEGORY_DISPLAY_NAMES = {
    "dryer": "Clothes Dryer", "washer": "Clothes Washer",
    "laundry center": "Laundry Center", "fridge": "Refrigerator",
    "refrig": "Refrigerator", "range": "Range", "oven": "Oven",
    "cooktop": "Cooktop", "microwave": "Microwave Oven",
    "mocrowave": "Microwave Oven", "beverage center": "Beverage Center",
    "coffee maker": "Coffee Maker", "espresso": "Espresso Machine",
    "toaster": "Toaster", "freezer": "Freezer",
}

SYSTEM_PROMPT_TEMPLATE = """You are a product-data enrichment specialist for Unilog, \
...
content for a {category} product record.
...
Professional/Gallery lines (parent company Rheem/Electrolux family)
  - MPN prefixes like "WDT", "WDTS", "WDF", "DR", "DF", "FF" commonly indicate \
Whirlpool or Speed Queen depending on other context
  - MPN prefixes like "LDPH", "LDT", "LDF", "WKE" commonly indicate LG
  - MPN prefixes like "KDFM", "KDTS", "KDPS", "KDTM" commonly indicate KitchenAid
  - MPN prefixes like "C7", "C9", "PEP", "CHP", "XOU" commonly indicate GE / GE Café / GE Profile
...
covering series, key performance specs, voltage/amperage where relevant, \
mounting/install type, dimensions, sound level (if applicable), material, \
...
- Use standard UOM abbreviations (V, A, in, dBA, kW-hr, hr, cu ft) — never full words.
- Do not fabricate certifications, colors, or model-specific numbers you are \
not reasonably confident about.
- Attribute labels should be appropriate to a {category} specifically (e.g. a \
dryer cares about drum capacity and drying technology, a refrigerator cares \
about total capacity and door configuration, a cooktop cares about burner \
count and fuel type) — don't reuse dishwasher-specific labels for a different \
category.
...
Product category (already classified): {category}
...
def call_llm(row, category_display):
    system_prompt = SYSTEM_PROMPT_TEMPLATE.format(category=category_display)
...
def merge_into_skeleton(skeleton_record, llm_output):
    ...
    record["Product Name"] = llm_output.get("product_name", "")
...
def main():
    ...
    with open(TIER2_RAW_PATH, encoding="utf-8") as f:
    ...
    if os.path.exists(OUTPUT_PATH):
        ...
        for rec in existing:
            ...
            if mpn:
                completed[mpn] = rec
        print(f"Resuming: {len(completed)} rows already completed, will skip them.\n")

    enriched = list(completed.values())
    already_done_mpns = set(completed.keys())

    for i, (skeleton_record, raw_row) in enumerate(zip(skeleton_records, raw_rows)):
        mpn = raw_row.get("Mfg_Part_Num", "")

        if mpn in already_done_mpns:
            continue

        category_key = category_map.get(mpn, "UNCLASSIFIED")
        category_display = CATEGORY_DISPLAY_NAMES.get(category_key, "Major Appliance")

        print(f"[{i+1}/{len(raw_rows)}] Enriching {mpn} ({category_display}) ...")

        try:
            llm_output = call_llm(raw_row, category_display)
        except Exception as e:
            error_name = type(e).__name__
            if "RateLimit" in error_name:
                print(f"\n⚠ Rate limit hit after {len(enriched)} rows. "
                      f"Progress saved — rerun this script later to continue "
                      f"from where it left off.\n  ({e})")
            else:
                print(f"\n⚠ Unexpected error after {len(enriched)} rows: {e}\n"
    ...
    print(f"\nDone (or paused). {len(enriched)}/{len(raw_rows)} rows saved -> {OUTPUT_PATH}")
    if len(enriched) < len(raw_rows):
        print("Rerun this script to continue with the remaining rows.")
